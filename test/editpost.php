...
if (isset($_REQUEST["update-btn"]))
{
    ...
    if($check != false) {
        ...
        $addnews  = "UPDATE `news` SET `news_headline`=\"$headline\",`news_description`=\"$description\",`image_name`=\"$newname\",`top_news`=\"$topnews\",`latest_news`= \"$latestnews\" WHERE `news_id`= \"$editnewsid\";";
    }
    else{
        $addnews  = "UPDATE `news` SET `news_headline`=\"$headline\",`news_description`=\"$description\",`top_news`=\"$topnews\",`latest_news`= \"$latestnews\" WHERE `news_id`= \"$editnewsid\";";
    }
...
<div class="container-fluid">
    ...
    <div class="form-group">
        ...
        <textarea class="form-control" placeholder="Write News/Post Headline" name="newsheadline" id="exampleFormControlTextarea1" rows="3" maxlength="190" required><?php echo($newsdetailsresult["news_headline"]); ?></textarea>
    </div>
    <div class="form-group">
    ...
    <div class="form-check col-auto mt-2">
     <p class="ml-1 mt-3">
        ...
        Latest News = Label news as <span class="badge badge-success">New</span> 
    </p>
    </div>
